...
class ITargetVisitor(IVisitor[TargetType], Generic[TargetType]):
    ...
    def _set_target_properties(self,
        ...
        @param generator The CMake generator to add code to.
        """
        # Set the non-configuration specific target properties
        self._set_properties(
            target,
            [
                (
                    "CXX_CLANG_TIDY",
                    properties.clang_tidy
                ),
                (
                    "CXX_CLANG_TIDY_EXPORT_FIXES_DIR",
                    properties.clang_tidy_export_fixes_dir
                ),
                (
                    "CXX_COMPILER_LAUNCHER",
                    properties.compiler_launcher
                ),
                (
                    "CXX_CPP_CHECK",
                    properties.cpp_check
                ),
                (
                    "CXX_CPP_LINT",
                    properties.cpp_lint
                ...
                (
                    "CXX_LINKER_LAUNCHER",
                    properties.linker_launcher
                ),
                (
                    "CXX_VISIBILITY_PRESET",
                    properties.visibility_preset
                ),
                (
                ...
                (
                    ...
                    Traced(True, properties.cxx_standard.origin),
                ),
                (
                    "EXCLUDE_FROM_ALL",
                ...
                (
                    "IMPORTED_LIBNAME",
                    properties.imported_name
                ),
                (
                    "IMPORTED_IMPLIB_NAME",
                    properties.imported_implib_name
                ),
                (
                    "IMPORTED_LOCATION",
    ...
    def _set_properties(self,
        ...
        def has_value(p: Traced[Optional[Any]] | Iterable[Traced[Any]]) -> bool:
            if isinstance(p, Traced):
                ...
                return True
        set_properties = filter(lambda p: has_value(p[1]), properties)
