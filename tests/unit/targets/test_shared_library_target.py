...
class TestSharedLibraryTarget:
    ...
    def test_shared_library_target_sets_target_type_correctly(self):
        target = SharedLibraryTarget(self.build_script_set, "foo")
        assert target.target_type == ETargetType.SHARED


    def test_generate_cmake_code(self):
        """
        Verifies that creating a shared target generates the appropriate CMake
          code.
        """
        target_name = "foo"
        target = SharedLibraryTarget(self.build_script_set, target_name)
    ...
    def test_trace_file_includes_all_properties(self, tmp_path: Any):
        ...
        assert ETargetType.SHARED.value in contents
