...
class TestStaticLibraryTarget:
    ...
    def test_static_library_target_sets_target_type_correctly(self):
        target = StaticLibraryTarget(self.build_script_set, "foo")
        assert target.target_type == ETargetType.STATIC


    def test_generate_cmake_code(self):
        """
        Verifies that creating a static target generates the appropriate CMake
          code.
        """
        target_name = "foo"
        target = StaticLibraryTarget(self.build_script_set, target_name)
    ...
    def test_trace_file_includes_all_properties(self, tmp_path: Any):
        ...
        assert ETargetType.STATIC.value in contents
